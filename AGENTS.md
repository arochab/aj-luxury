...
- Décision d’Adam du 11 août 2026 : chaque release publique approuvée doit être visible depuis le `.com` et le `.fr`. L’implémentation permanente est une seule release applicative sur le `.com`, puis une redirection HTTPS `301` ou `308` de l’apex et de `www.ajluxurystore.fr` vers le `.com`. Ne jamais maintenir deux déploiements ou contenus divergents. Tant que la redirection n’est pas active, signaler le `.fr` comme gate séparé et ne pas prétendre que la release y est disponible.
