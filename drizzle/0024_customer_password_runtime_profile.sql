-- Preserve any historical credential rows while admitting the Workers-safe,
-- OWASP-equivalent PBKDF2-HMAC-SHA-512 profile used for all new passwords.
...
RENAME TO `customer_password_credentials_legacy_0024`;--> statement-breakpoint
...
CREATE TABLE `customer_password_credentials` (
  ...
  CONSTRAINT `ck_customer_password_hash` CHECK(
    (
      (`algorithm` = 'pbkdf2-sha512' AND `iterations` = 220000)
...
SELECT
  `customer_id`,`algorithm`,`iterations`,`salt_base64url`,`hash_base64url`,
  `failed_attempts`,`locked_until`,`password_changed_at`,`created_at`,`updated_at`
FROM `customer_password_credentials_legacy_0024`;--> statement-breakpoint

DROP TABLE `customer_password_credentials_legacy_0024`;--> statement-breakpoint
