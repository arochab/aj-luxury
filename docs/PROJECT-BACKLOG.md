...
| AJ-111 | Configurer mesure et consentement | À connecter | Adam CHABBI | Façade client inactive et bundle sans `order_paid` ; chaque fichier serveur profond non bundleable au navigateur ; 12 variantes `AJ-APO` lues depuis le catalogue commerce réel ; `order_paid` explicitement indisponible jusqu’à la D1 commerce canonique ; typecheck racine sans nouveau diagnostic Lot 2 | `docs/ANALYTICS-LOT-2.md` et tests automatisés | AJ-110 |
